...
public:
    ...
    void halfSpace(int x, int y)
    {
        // 座標中心化
        x /= _step; x = x * _step + _step/2;
        y /= _step; y = y * _step + _step/2;
        point p = {x, y};
        _pointVector.push_back(p);

        _half_space_point_count ++;
        if (_half_space_point_count == 3)
        {
            for (unsigned int i = 0; i < _pointVector.size(); i++)
            {
                for (unsigned int j = i+1; j < _pointVector.size(); j++)
                {
                    Func f;
                    if (j == _pointVector.size()-1 && i == 0) // 最後一個點方向往起始點
                    {
                        f = Func(_pointVector[j].x, _pointVector[j].y, 
                                      _pointVector[i].x, _pointVector[i].y);
                    }
                    else
                    {
                        f = Func(_pointVector[i].x, _pointVector[i].y, 
                                      _pointVector[j].x, _pointVector[j].y);
                    }
                    
                    _funcVector.push_back(f);
                }
            }
            ...
            for (Func func:_funcVector) func.showFunc();

            for (float i = _step/2; i < 500 ; i += _step)
            {
                for (float j = _step/2; j < 500 ; j += _step)
                {
                    if (_funcVector[0].isLeftSide(i, j) &&
                    _funcVector[1].isLeftSide(i, j) &&
                    _funcVector[2].isLeftSide(i, j))
                    {
                        this->addSqr(i, j, 1);
                    }
...
private:
    ...
    int _half_space_point_count;

    vector<Sqr> _sqrVector;
    vector<point> _pointVector;
    vector<Func> _funcVector;
};
